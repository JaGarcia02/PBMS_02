...
import HrUpdateContractType from "./HrUpdateEmploymentType";
import { Logs } from "../../../utils/Logs";

const HrAddEmploymentType = ({ setAddContractType, setDataCategory }) => {
  ...
  // Notify Create
  const notify_Create = () => {
    toast.success("Employment Category Created", {
  ...
  const sumbit_newContractType = (e) => {
    e.preventDefault();
    if (newCategory.trim() == "") {
      setCheckEmpty(true);
    } else {
      axios
        .post(API_URL_HR + "add-employment-category", {
    ...
    >
      <motion.div className="absolute bg-white h-80 w-120 items-center shadow-md shadow-gray-900 z-999">
        <div className="w-full h-full flex flex-col item-center text-center mb-5">
          <div className=" prdc-color w-full h-25">
            ...
            <div className="flex items-center">
              ...
              <AiOutlineCloseSquare
                onClick={() => setAddContractType(false)}
          ...
          <form className="w-full" onSubmit={sumbit_newContractType}>
...
export default HrAddEmploymentType;
